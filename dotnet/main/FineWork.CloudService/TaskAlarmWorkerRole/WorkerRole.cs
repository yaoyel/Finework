        ...
        private async Task RunAsync(CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                using (var tx = TxManager.Acquire())
                {
                    ...
                    var anncs = m_AnnouncementManager.FetchAnncByEndTime(DateTime.Now).ToList();

                    if (!alarmPeriod.Any() && !anncs.Any())
                    {
                        ...
                        await Task.Factory.StartNew(() =>
                        {
                            ...
                            alarmPeriod.ForEach(p =>
                            {
                                ...
                                partakers.ToList().ForEach(s =>
                                {
                                    ...
                                    m_NotificationManager.SendByAliasAsync(null, $"您收到一个定时预警信息。", customizedValue,
                                        s.Staff.Account.PhoneNumber);
                                });
                    ...
                    if (anncs.Any())
                        await Task.Factory.StartNew(() =>
                        {
                            anncs.ForEach(p =>
                            {
                                ...
                                var customizedValue = new Dictionary<string, string>()
                                {
                                    ["AnncId"] = p.Id.ToString(),
                                    ["PathTo"] = "Annc",
                                    ["StaffId"] = receiver.Id.ToString(),
                                    ["OrgId"] = receiver.Org.Id.ToString()
                                };

                                m_NotificationManager.SendByAliasAsync(null, $"您收到一个里程碑验证信息。", customizedValue,
                                    receiver.Account.PhoneNumber);
                                Trace.TraceInformation(
                                    $"{DateTime.Now.ToString("yyyy/MM/dd hh:mm:ss")}发送一个里程碑验证消息至{receiver.Name}。");
                            });
